...
class Temperature(object):
    ...
    def enable_heater(self, event):
        self.control.setHeater(self.heater_var.get())
    ...
    def record(self):
        ...
        PID = [self.P_var.get(), self.I_var.get(), self.D_var.get()]
    ...
    def create_interface(self):
        ...
        self.heater_options = ['OFF', 'LOW', 'MED', 'HIGH']
        self.heater_var = tk.StringVar()
        self.heater_var.set(self.heater_options[0])
        #self.heaterButton_var = tk.StringVar()
        #self.heaterButton_var.set('Enable heater')
        ...
        ttk.Label(master=set_frame, text="P:").grid(column=0, row=4, sticky=tk.EW)
        ttk.Label(master=set_frame, text="I:").grid(column=0, row=5, sticky=tk.EW)
        ttk.Label(master=set_frame, text="D:").grid(column=0, row=6, sticky=tk.EW) 
        ...
        ttk.Entry(master=set_frame, width=10, textvariable=self.P_var).grid(column=1, row=4, sticky=tk.EW)
        ttk.Entry(master=set_frame, width=10, textvariable=self.I_var).grid(column=1, row=5, sticky=tk.EW)
        ttk.Entry(master=set_frame, width=10, textvariable=self.D_var).grid(column=1, row=6, sticky=tk.EW)

        ttk.Button(master=set_frame, width=10, text='Set', command=self.updateSetpoint).grid(column=1, row=7, sticky=tk.EW)
        #ttk.Button(master=set_frame, width=10, textvariable=self.heaterButton_var, command=self.enable_heater).grid(column=0, row=8, sticky=tk.EW)
        tk.OptionMenu(set_frame, self.heater_var, *self.heater_options, command=self.enable_heater).grid(column=0, row=7, sticky=tk.EW)
