...
class CV:
    ...
    def create_menu_bar(self):
        ...
        if not self.in_batch:
            ...
            self.master.menu_batch.add_command(label='CV_batch', command=lambda: self.new_batch('CV_batch'))
    ...
    def create_interface(self):
        ...
        self.start_var.set('20')
        self.start_label = ttk.Label(sweep_frame, text='Frequency start (Hz): ')
        self.start_label.grid(column=0, row=0, sticky=(tk.E, tk.W, tk.S))
        ttk.Entry(master=sweep_frame, width=10, textvariable=self.start_var).grid(column=1, row=0, sticky=(tk.E, tk.W, tk.S))

        self.stop_var = tk.StringVar()
        self.stop_var.set('1e07')
        self.stop_label = ttk.Label(sweep_frame, text='Frequency stop (Hz): ')
        self.stop_label.grid(column=0, row=1, sticky=(tk.E, tk.W, tk.S))
        ttk.Entry(master=sweep_frame, width=10, textvariable=self.stop_var).grid(column=1, row=1, sticky=(tk.E, tk.W, tk.S))
    ...
    def select_za(self, *args):
        ...
        dev_name = self.za_var.get() #works
    ...
    def mode(self):
        ...
        if self.fixed_var.get() == 'bias':
            self.start_label['text'] = 'Frequency start (Hz): '
            self.stop_label['text'] = 'Frequency stop (Hz): '
            self.start_var.set('20')
            self.stop_var.set('1e7')
            ...
            self.start_label['text'] = 'Bias start (V): '
            self.stop_label['text'] = 'Bias stop (V): '
            self.start_var.set('-0.5')
            self.stop_var.set('0.5')
    ...
    def prepare_scan(self):
        ...
        :return: None
        """
        mode = self.fixed_var.get()
        fixed_value = float(self.fixed_value_var.get())
        start = float(self.start_var.get())
        stop = float(self.stop_var.get())
        nop = int(self.nop_var.get())
        navg = int(self.navg_var.get())        
        osc_amp = float(self.osc_amp_var.get())
